...
if( isset( $_POST['submit'] ) ){
	...
	if( 0 < strlen( $token ) ){//トークン決済
		...
		$execInput->setToken( $token );
		$execInput->setTokenType($tokenType);

	}else if( 0 < strlen( $memberId )  ){//会員ID決済
	...
	if( $exe->isExceptionOccured() ){//取引の処理そのものがうまくいかない（通信エラー等）場合、例外が発生します。
		...
		if( $output->isErrorOccurred() ){//出力パラメータにエラーコードが含まれていないか、チェックしています。
			...
			require_once( BASEPATH.DIRECTORY_SEPARATOR.'src/com/gmo_pg/client/input/AcsParam.php');
			require_once( BASEPATH.DIRECTORY_SEPARATOR.'src/com/gmo_pg/client/common/RedirectUtil.php');
