...
if( isset( $_POST['submit'] ) ){
	...
	$input = new SearchTradeMultiInput();/* @var $input SearchTradeMultiInput */
	...
	$input->setPayType( $_POST['PayType'] );
	
	//API通信クラスをインスタンス化します
	$exe = new SearchTradeMulti();/* @var $exec SearchTradeMulti */
	
	//パラメータオブジェクトを引数に、実行メソッドを呼び、結果を受け取ります。
	$output = $exe->exec( $input );/* @var $output SearchTradeMultiOutput */
...
//SearchTradeMulti入力・結果画面
require_once( PGCARD_SAMPLE_BASE . '/display/SearchTradeMulti.php' );