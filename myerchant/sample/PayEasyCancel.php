...
if( isset( $_POST['submit'] ) ){
	require_once( 'com/gmo_pg/client/input/PayEasyCancelInput.php');
	require_once( 'com/gmo_pg/client/tran/PayEasyCancel.php');

	//入力パラメータクラスをインスタンス化します
	$input = new PayEasyCancelInput();/* @var $input PayEasyCancelInput */
	...
	//API通信クラスをインスタンス化します
	$exe = new PayEasyCancel();/* @var $exec PayEasyCancel */

	//パラメータオブジェクトを引数に、実行メソッドを呼び、結果を受け取ります。
	$output = $exe->exec( $input );/* @var $output PayEasyCancelOutput */
...
//PayEasyCancel入力・結果画面
require_once( PGCARD_SAMPLE_BASE . '/display/PayEasyCancel.php' );