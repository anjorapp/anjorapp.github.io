...
if( isset( $_POST['submit'] ) ){
	...
	$input = new ExecTranPaidInput();/* @var $input ExecTranPaidInput */
	...
	$input->setContents( mb_convert_encoding( $_POST['Contents'] , 'SJIS' , PGCARD_SAMPLE_ENCODING ) );


	//API通信クラスをインスタンス化します
	$exe = new ExecTranPaid();/* @var $exec ExecTranPaid */

	//パラメータオブジェクトを引数に、実行メソッドを呼び、結果を受け取ります。
	$output = $exe->exec( $input );/* @var $output ExecTranPaidOutput */
...
//ExecTranPaid入力・結果画面
require_once( PGCARD_SAMPLE_BASE . '/display/ExecTranPaid.php' );