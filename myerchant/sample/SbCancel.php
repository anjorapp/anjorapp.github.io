...
if( isset( $_POST['submit'] ) ){
	...
	$input = new SbCancelInput();/* @var $input SbCancelInput */
	...
	//API通信クラスをインスタンス化します
	$exe = new SbCancel();/* @var $exec SbCancel */

	//パラメータオブジェクトを引数に、実行メソッドを呼び、結果を受け取ります。
	$output = $exe->exec( $input );/* @var $output SbCancelOutput */
...
//SbCancel入力・結果画面
require_once( PGCARD_SAMPLE_BASE . '/display/SbCancel.php' );