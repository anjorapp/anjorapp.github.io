...
if( isset( $_POST['submit'] ) ){
	require_once( 'com/gmo_pg/client/input/PaypalSalesInput.php');
	require_once( 'com/gmo_pg/client/tran/PaypalSales.php');
	
	//入力パラメータクラスをインスタンス化します
	$input = new PaypalSalesInput();/* @var $input PaypalSalesInput */
	...
	//API通信クラスをインスタンス化します
	$exe = new PaypalSales();/* @var $exec PaypalSales */
	
	//パラメータオブジェクトを引数に、実行メソッドを呼び、結果を受け取ります。
	$output = $exe->exec( $input );/* @var $output PaypalSalesOutput */
...
//PaypalSales入力・結果画面
require_once( PGCARD_SAMPLE_BASE . '/display/PaypalSales.php' );