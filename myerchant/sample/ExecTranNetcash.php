...
if( isset( $_POST['submit'] ) ){
	...
	$input = new ExecTranNetcashInput();/* @var $input ExecTranNetcashInput */
	...
	//API通信クラスをインスタンス化します
	$exe = new ExecTranNetcash();/* @var $exec ExecTranNetcash */

	//パラメータオブジェクトを引数に、実行メソッドを呼び、結果を受け取ります。
	$output = $exe->exec( $input );/* @var $output ExecTranNetcashOutput */
...
//ExecTranNetcash入力・結果画面
require_once( PGCARD_SAMPLE_BASE . '/display/ExecTranNetcash.php' );