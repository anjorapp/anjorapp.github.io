...
if( isset( $_POST['submit'] ) ){
	require_once( 'com/gmo_pg/client/input/RegisterRecurringAccounttransInput.php');
	require_once( 'com/gmo_pg/client/tran/RegisterRecurringAccounttrans.php');

	//入力パラメータクラスをインスタンス化します
	$input = new RegisterRecurringAccounttransInput();/* @var $input RegisterRecurringAccounttransInput */
	...
	$input->setPrintStr( mb_convert_encoding( $_POST['PrintStr'] , 'SJIS' , PGCARD_SAMPLE_ENCODING ) );


	//API通信クラスをインスタンス化します
	$exe = new RegisterRecurringAccounttrans();/* @var $exec RegisterRecurringAccounttrans */

	//パラメータオブジェクトを引数に、実行メソッドを呼び、結果を受け取ります。
	$output = $exe->exec( $input );/* @var $output RegisterRecurringAccounttransOutput */
...
//RegisterRecurringAccounttrans入力・結果画面
require_once( PGCARD_SAMPLE_BASE . '/display/RegisterRecurringAccounttrans.php' );