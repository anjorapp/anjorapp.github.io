...
if( isset( $_POST['submit'] ) ){
	require_once( 'com/gmo_pg/client/input/SbSalesInput.php');
	require_once( 'com/gmo_pg/client/tran/SbSales.php');

	//入力パラメータクラスをインスタンス化します
	$input = new SbSalesInput();/* @var $input SbSalesInput */
	...
	//API通信クラスをインスタンス化します
	$exe = new SbSales();/* @var $exec SbSales */

	//パラメータオブジェクトを引数に、実行メソッドを呼び、結果を受け取ります。
	$output = $exe->exec( $input );/* @var $output SbSalesOutput */
...
//SbSales入力・結果画面
require_once( PGCARD_SAMPLE_BASE . '/display/SbSales.php' );