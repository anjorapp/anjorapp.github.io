...
if( isset( $_POST['submit'] ) ){
	...
	$input = new ExecTranEdyInput();/* @var $input ExecTranEdyInput */
	...
	$input->setEdyAddInfo1( mb_convert_encoding( $_POST['EdyAddInfo1'] , 'SJIS' , PGCARD_SAMPLE_ENCODING ) );
	$input->setEdyAddInfo2( mb_convert_encoding( $_POST['EdyAddInfo2'] , 'SJIS' , PGCARD_SAMPLE_ENCODING ) );
	...
	//API通信クラスをインスタンス化します
	$exe = new ExecTranEdy();/* @var $exec ExecTranEdy */
	
	//パラメータオブジェクトを引数に、実行メソッドを呼びます。
	//正常に終了した場合、結果オブジェクトが返るはずです。
	$output = $exe->exec( $input );/* @var $output ExecTranEdyOutput */
...
//ExecTranEdy入力・結果画面
require_once( PGCARD_SAMPLE_BASE . '/display/ExecTranEdy.php' );