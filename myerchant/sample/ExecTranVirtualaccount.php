...
if( isset( $_POST['submit'] ) ){
	...
	$input = new ExecTranVirtualaccountInput();/* @var $input ExecTranVirtualaccountInput */
	...
	//API通信クラスをインスタンス化します
	$exe = new ExecTranVirtualaccount();/* @var $exec ExecTranVirtualaccount */

	//パラメータオブジェクトを引数に、実行メソッドを呼び、結果を受け取ります。
	$output = $exe->exec( $input );/* @var $output ExecTranVirtualaccountOutput */
...
//ExecTranVirtualaccount入力・結果画面
require_once( PGCARD_SAMPLE_BASE . '/display/ExecTranVirtualaccount.php' );